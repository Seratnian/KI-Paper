...
public class TrackValues : MonoBehaviour
{
    ...
    public int discreteAreaXInto = 100;
    public int discreteAreaYInto = 100;
    ...
    private readonly float DISTANCE_TO_TARGET = 10;
    ...
    {
        string splitTime = this.splitTime;
        this.splitTime = "";
        return splitTime;
    }
        ...
        for (int x = -discreteDistance; x <= discreteDistance; x++)
        {
            for (int y = -discreteDistance; y <= discreteDistance; y++)
            {
                if (Math.Abs(x) == discreteDistance || Math.Abs(y) == discreteDistance)
                {
                    ...
                    if (finalX >= 0 && finalX < discreteAreaXInto && finalY >= 0 && finalY < discreteAreaYInto)
    ...
    private Vector2 calculatePosition(Transform of)
    {
		...
		position.x = (int)Math.Floor (of.position.x / sizeOfGround.x * discreteAreaXInto);
		position.y = (int)Math.Floor (of.position.z / sizeOfGround.z * discreteAreaYInto);
        return position;
    }
	public int collx(Transform of)
	{
		int positionx = (int)Math.Floor (of.position.x / sizeOfGround.x * discreteAreaXInto);
		return positionx;
	}
	public int colly(Transform of)
	{
		int positiony = (int)Math.Floor (of.position.z / sizeOfGround.z * discreteAreaYInto);
		return positiony;
	}
    ...
    private void OnTriggerEnter(Collider collider)
    {
        if (trackSplitTimes)
        {
            foreach (Transform timer in whereToTrack)
            {
                if (timer.name == collider.name)
                {
                    splitTime = timer.name;
                    break;
                }
            }
        }
